...
const RegisterScreen = () => {
    ...
    const handleData = (values) => {
        mutate(`${process.env.REACT_APP_API_URL}/register`, {
            ...
            data: values,
            onSuccess: () => {
                window.location.href = route(AuthRoutes.Login);
            },
        });
    };
    ...
    return (
        <div className="w-full h-screen relative flex justify-center items-center grow-0">
            ...
            <div className="w-4/5 bg-white md:w-2/3 lg:w-2/5 rounded-xl px-10 py-12 mt-20">
                <div className="text-center flex items-center flex-col">
                    <img width="130" src="/logo-color.png" alt="logo" />
                    <h1 className="mt-6 text-gray-900 text-2xl font-bold">{t("onboarding.register.title")}</h1>
                </div>
                <form className="mt-8 space-y-6" onSubmit={handleSubmit(handleData)} noValidate={true}>
                    ...
                    <p className="mt-2 text-center text-sm text-gray-600">
                        {t("onboarding.register.account")}
                        <Link to={route(AuthRoutes.Login)} className="font-medium text-indigo-600 hover:text-indigo-500"> {t("onboarding.register.link")} </Link>
                    </p>
                </form>
            </div>
        </div >
